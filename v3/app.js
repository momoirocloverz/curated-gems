...
function mountControls() {
  ...
  const texts = {
    ...
    en: {
      ...
      tagHint:
        '💡 Tip: Click tags to filter content, you can select multiple tags',
...
function updateFilterStatus() {
  ...
  const texts = {
    zh: `显示 ${filteredCount} / ${totalCount} 篇文章${
      activeFiltersCount > 0 ? ` (${activeFiltersCount} 个筛选条件)` : ''
    }`,
    en: `Showing ${filteredCount} / ${totalCount} articles${
...
function renderSources(list) {
  ...
  el.innerHTML = `
    ...
    }" data-value="all" onclick="toggleMulti(event, 'source')">
      ${allText} (${list.length})
    </div>
    ${sources
      .map((source) => {
        ...
        }" data-value="${source}" onclick="toggleMulti(event, 'source')" title="${statusText}">
          ${esc(source)} (${count})
        </div>
      `
      })
...
function renderTags(list) {
  ...
  const tagsField = lang === 'zh' ? 'tags_zh' : 'tags'
  const allTags = [
    ...new Set(list.flatMap((item) => item[tagsField] || item.tags || [])),
  ]
  ...
  $('#tags').innerHTML = tags
    .map((t) => {
      ...
      // TODO: 在这里添加标签数量显示逻辑
      return `<span class="tag ${isActive ? 'active' : ''}" data-tag="${esc(
        tagValue
      )}">
...
function showWelcomeGuide() {
  ...
  const welcomeTexts = {
    zh: {
      ...
      content:
        '这里有 {count} 条精选内容等你探索！<br><br>💡 <strong>快速上手指南：</strong><br>• 使用搜索框快速查找内容<br>• 点击标签进行筛选（可多选）<br>• 试试随机推荐发现新内容<br>• 完成任务文档中的4个学习任务',
      button: '开始探索',
      taskLink: '查看学习任务',
    },
    en: {
      ...
      content:
        'There are {count} curated items waiting for you to explore!<br><br>💡 <strong>Quick Start Guide:</strong><br>• Use search box to find content quickly<br>• Click tags to filter (multiple selection)<br>• Try random recommendation to discover new content<br>• Complete 4 learning tasks in the task document',
...
function showTagStatistics() {
  ...
  const avgTags = (
    Object.values(tagStats).reduce((a, b) => a + b, 0) / totalItems
  ).toFixed(1)
...
function card(item, lang = 'zh') {
  const tagsArray =
    lang === 'zh' ? item.tags_zh || item.tags || [] : item.tags || []
  ...
  return `
    <article class="card" data-id="${item.id}">
      <h3><a href="${item.link}" target="_blank" rel="noopener">${esc(
    title
  )}</a></h3>
      ${
        desc
      ...
      ${
        quote
          ? `<blockquote>${quoteSymbols[0]}${esc(quote)}${
              quoteSymbols[1]
            }</blockquote>`
          : ''
      }
